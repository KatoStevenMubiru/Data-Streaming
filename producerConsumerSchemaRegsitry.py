...
class ClickAttribute:
    ...
    @classmethod
    def attributes(cls):
        return {faker.uri_page(): cls() for _ in range(random.randint(1, 5))}
...
async def produce(topic_name):
    ...
    p = AvroProducer(
        ...
        default_value_schema=ClickEvent.schema
    )
    while True:
...
async def consume(topic_name):
    ...
    c = AvroConsumer(
        {"bootstrap.servers": BROKER_URL, "group.id": "0", "schema.registry.url": SCHEMA_REGISTRY_URL},
