...
CONNECTOR_NAME = "ksm"  # Updated connector name to "ksm"
...
def configure_connector():
    ...
    resp = rest_method(
        ...
        data=json.dumps(
            {
                "name": "KSM",  # Set the connector name to "ksm"
                "config": {
                    "connector.class": "FileStreamSource",  # TODO: Set the connector class
                    "topic": "ksm-topic",  # TODO: Set the Kafka topic for "ksm"
                    "tasks.max": 1,  # TODO: Set the maximum number of tasks
...
async def log():
    ...
    with open(f"/tmp/{CONNECTOR_NAME}.log", "a") as f:  # Use "a" for append mode
        iteration = 0
        while True:
